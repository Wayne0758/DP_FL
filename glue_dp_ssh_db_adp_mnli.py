...
def parse():
    ...
    parser.add_argument('--num_local_updates',            default=3,                 type=int)
...
def main():
    ...
    config = AutoConfig.from_pretrained(
        f"./db-config/{our_args.task_name}",
        num_labels=num_labels,
        finetuning_task=our_args.task_name,
    )

    tokenizer = AutoTokenizer.from_pretrained(
        f"./db-tokenizer/{our_args.task_name}",
        do_lower_case=False,
    )

    model = AutoModelForSequenceClassification.from_pretrained(
        f"./db-model/{our_args.task_name}",
        config=config,
    ...
    os.makedirs("adp_mnli_model_checkpoints", exist_ok=True)
    ...
    for epoch in range(1, our_args.num_communication_rounds+1):
        ...
        checkpoint_path = os.path.join(
            "adp_mnli_model_checkpoints", 
            f"adp_mnli_model_epoch_{epoch}_{our_args.task_name}.pt"
        )
