...
def parse():
    ...
    parser.add_argument('--model_name_or_path',           default='distilbert-based-uncased',     type=str)
    ...
    parser.add_argument('--private',                      default=True,              type=bool)
...
def main():
    ...
    def generate_cyclic_mixing_matrix(n):
        ...
        for i in range(n):
            ...
            M[i, [previous, current, next_user]] = 1 / 3
    ...
    W = generate_cyclic_mixing_matrix(our_args.num_clients)
    if our_args.num_clients == 1:
        W = np.ones((1, 1))
    ...
    for epoch in range(1, our_args.num_communication_rounds+1):
        ...
        for client in range(our_args.num_clients):
            ...
            for LU in range(1, our_args.num_local_updates+1):
                with BatchMemoryManager(
                    data_loader=train_dataloader[client],
                    max_physical_batch_size=our_args.batch_size,
        ...
        for i in range(our_args.num_clients):
            for key in local_weights[0].keys():
                local_weights[i][key] = sum(W[i, j] * local_weights[j][key] for j in range(our_args.num_clients))
        
        global_peft_weights = {}
        for key in local_weights[0].keys(): 
            global_peft_weights[key] = sum(local_weights[i][key] for i in range(our_args.num_clients)) / our_args.num_clients
