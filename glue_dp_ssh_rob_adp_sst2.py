...
def main():
    ...
    config = AutoConfig.from_pretrained(
        f"./roberta-base-config/sst2",
        num_labels=num_labels,
        finetuning_task=our_args.task_name,
    )

    tokenizer = AutoTokenizer.from_pretrained(
        f"./roberta-base-tokenizer/sst2",
        do_lower_case=False,
    )

    model = AutoModelForSequenceClassification.from_pretrained(
        f"./roberta-base-model/sst2",
        config=config,
                ...
                if our_args.EPSILON == float("inf"):
                    ...
                    for step, batch in enumerate(tqdm(memory_safe_data_loader)):
                        ...
                        if step > 0 and step % our_args.LOGGING_INTERVAL == 0:
                            ...
                            eps = eps = privacy_engine.get_epsilon(our_args.DELTA) if our_args.private else float('inf')
